...
let container = new ProductManager('./src/models/products.json');
...
const getProducts = async (req, res) => {
    try{
        ...
        if(limit != null || limit > 0) {
            ...
            res.render('home', {productsLimit, style});
...
const getProductId = (req,res) => {
    ...
    container.getById(parseInt(pid)).then(product => res.render('home', {product, style})); //res.json(product));
};
...
const saveProduct = (req, res) => {
    ...
    container.save(newProduct).then(id => {
        if(id!=null){
            container.getById(id).then(product => res.render('home', {product, style}));//res.json(product))
        }else{
...
const updateProduct = (req, res) => {
    ...
    container.updateProduct(newProduct).then(() => container.getById(pid).then((prod) => res.render('home', {prod, style})));//res.json(prod)));
}

const deleteProduct = (req, res) => {
    const { pid } = req.params;
    container.deleteById(parseInt(pid)).then(() => container.getAll().then(products => res.render('home', {products, style})));//res.json(products)))
}
